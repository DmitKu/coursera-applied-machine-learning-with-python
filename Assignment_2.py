...
def answer_two():
    ...
    for i in range(0,10):
        ...
        linreg_model = LinearRegression().fit(X_train_poly, y_train)

        r2_train[i] = linreg_model.score(X_train_poly, y_train)

        

        X_test_poly = poly.fit_transform(X_test.reshape(-1, 1))

        r2_test[i] = linreg_model.score(X_test_poly, y_test)
...
def answer_four():
    ...
    LinearRegression_R2_test_score = linreg_model.score(X_test_poly, y_test);
...
def answer_five():
    ...
    DT_model = DecisionTreeClassifier(random_state=0).fit(X_train2, y_train2)

    features = []

    for index, importance in enumerate(DT_model.feature_importances_):
...
​
    ...
    return features # Your answer here
...
def answer_six():
    ...
    train_scores, test_scores = validation_curve(svc_model, X_subset, y_subset,
